...
export const calculateReadTimeInMinutes = (blogPostContent: BlogPostContent) => {
    ...
    blogPostContent.contentPieces.forEach((piece: BlogPostContentPieceAny) => {
        if (piece.type == "title") {
            totalNumberOfWordsInBlogPost += (piece as BlogPostContentTitle).title.length;
        }
        if (piece.type == "paragraph") {
            totalNumberOfWordsInBlogPost += (piece as BlogPostContentParagraph).text.length;
        }
