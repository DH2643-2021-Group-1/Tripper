...
const createBlogPost = async (req: express.Request, res: express.Response) => {
    try {
        var docRef = await createNewBlogPostDocInFirestore(req.body.title, req.body.description, req.body.userId);
    }
    catch (_) {
        respond(res, { statusCode: StatusCode.ErrorFirestoreCreate, error: "Could not create a new blogpost document in the database" });
        return;
    }

    const [primaryImage, imagePieces] = extractBlogPostImagesFromFileBody(req.files);

    if (primaryImage == null) {
        respond(res, { statusCode: StatusCode.RequestNoPrimaryImage, error: "Primary image is required"});
        errorCleanUp(docRef);
        return;
    }

    try {
        var primaryImageUrl = await uploadImage(primaryImage, `blogPostImages/${docRef.id}.png`);
    } catch (error) {
        respond(res, { statusCode: StatusCode.ErrorCouldNoUploadImage, error: "Could not upload the primary image"});
        errorCleanUp(docRef);
        return;
    } 
    
    try {
        var preparedContent = await prepareBlogPostContent(JSON.parse(req.body.content), imagePieces);
    } catch (error) {
        respond(res, { statusCode: StatusCode.ErrorBlogPostContentPreparation, error: "Could not prepare the blog post content for upload"});
        errorCleanUp(docRef);
        return;
    }
...
const updateBlogPost = async (req: express.Request, res: express.Response) => {
    try {
        var existingBlogPost = await db.collection('blogposts').doc(req.body.id).get();
    }
    catch (_) {
...
const prepareBlogPostContent = async (content: BlogPostContent, files: Express.Multer.File[] | null) => {
    await removeDeletedImagePiecesFromDatabase(content);
    filterOutDeletedContentPieces(content);
    
    if (files == null) return content;
    const contentWithImages = await uploadImagePiecesAndSetImageUrls(content, files);
    return resetContentEditTypes(contentWithImages);
}

const createNewBlogPostDocInFirestore = async (title: string, description: string, userId: string) => {
...
const removeDeletedImagePiecesFromDatabase = async (content: BlogPostContent) => {
    for (const piece of content.contentPieces) {
        if (piece.type == "image" && piece.editType == EditType.delete && piece.imageUrl != null) {
            try {
...
const uploadImagePiecesAndSetImageUrls = async (content: BlogPostContent, files: Express.Multer.File[]) => {
    let fileIndex = 0;
    for (const piece of content.contentPieces) {
...
const replacePrimaryImage = async (docRef: firestore.DocumentReference, newFile: Express.Multer.File) => {
